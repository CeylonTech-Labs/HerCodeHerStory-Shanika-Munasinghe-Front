...
export default function AdminCertificatesPage() {
  ...
  useEffect(() => { load(); }, []);
  return <div><AdminPageTitle title="Certificates" actionHref="/admin/certificates/create" actionLabel="Create certificate" /><DataTable data={items} columns={[{ header: "Title", cell: (i) => <b>{i.title}</b> }, { header: "Issuer", cell: (i) => i.issuer }, { header: "Date", cell: (i) => formatDate(i.issuedDate) }, { header: "Actions", cell: (i) => <div className="flex gap-2"><Button size="sm" variant="outline" asChild><Link href={`/admin/certificates/edit/${i.id}`}><Edit className="h-4 w-4" /></Link></Button><ConfirmDialog onConfirm={async () => { await deleteCertificate(i.id); await load(); toast.success("Deleted."); }}><Button size="sm" variant="secondary"><Trash2 className="h-4 w-4" /></Button></ConfirmDialog></div> }]} /></div>;
}
